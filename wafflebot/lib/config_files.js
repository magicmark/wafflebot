...
export default class ConfigFilesLoader {
     ...
     */
    _checkDirExists () {
        return this.fs.statAsync(this.resolvedConfigDirectory)
            .then(stats => {
                return stats.isDirectory();
            }).catch(err => {
                this.logger.debug(`${this.resolvedConfigDirectory} does not exist.`);
            });
    }
     ...
     * 
     * @param  {string} jsonFile - The file name
     ...
     */
    directoryReady () {
        return this._checkDirExists().then(exists => {
            if (!exists) {
